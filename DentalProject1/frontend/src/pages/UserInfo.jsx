...
function UserInfo() {
    ...
    const getDataa = async () => {
        try {
            const result = await axios.get(`${SummaryApi.singleUserForAdmin.url}${idd}`, {
                headers: {
    ...
    return (
        <div className="sm:flex sm:flex-col md:flex-row items-center p-5">
            ...
            <div className="w-full md:w-1/2 p-5">
                <Link to={"/UserTable"} className="inline-block mb-5 text-right w-full">
                ...
                <div className="text">
                    ...
                    {data?.role===0?"User":"Doctor"}     <span className="text-indigo-600">Profile</span>
                    </h2>
                    <form className="space-y-6">
                        ...
                        <div>
                            <label htmlFor="phone" className="block text-gray-700 text-sm font-medium">Role</label>
                            <input 
                                type="text" 
                                id="role" 
                                name="role" 
                                ...
                                placeholder="Enter your phone number" 
                                disabled 
                                value={data?.role===0?"User":"Doctor"} 
                            />
                        </div>
                        

                        {/* Commented out update button */}
                        {/* <div>
                            <button type="submit" className="w-full bg-indigo-600 text-white py-2 px-4 rounded-md focus:outline-none focus:ring-2 focus:ring-indigo-500 hover:bg-indigo-700">
                                Update Profile
                            </button>
                        </div> */}
                    </form>
